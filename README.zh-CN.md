...
- `council` 模式：让 Host Codex、Claude Code 和 OpenCode 尽量都参与同一个问题。
- `claude` 模式：只调用 Claude Code，把结果作为一个外部参考意见。
- `opencode` 模式：只调用 OpenCode，把结果作为一个外部参考意见。
...
安装完成后，Codex 的 slash 列表里只需要出现一个入口：`agent-council`。选中它之后，把模式写在最前面：
...
如果你调用 skill 时给了真实任务但没写模式，默认按 `council` 模式处理。如果你是在询问这个 skill 的行为、配置或排错，它会正常回答，不会调用外部 Agent。
...
把唯一的 skill 文件夹复制到 Codex skills 目录：
...
```text
/agent-council council 评估一下这个单体服务是否应该拆成计费、通知和报表三个服务。重点看迁移风险、团队复杂度和测试策略。
```

让 Claude Code 审核 Codex 写的 PR：

```text
/agent-council claude 帮我从第三方视角审查这个 PR，重点找隐藏的回归风险和缺失测试。只给外部参考意见，不需要做最终结论。
```

市场调研：

```text
/agent-council opencode 调研一个面向独立咖啡店的轻量会员系统是否有市场。比较目标用户、购买触发点、主要竞品和失败风险。
```

科研方向判断：

```text
/agent-council council 使用可用的文献检索能力，评估这个 XX 方向是否值得做三个月 pilot。请区分已有事实、模型推断和实际可行性。
```

文档结构建议：

```text
/agent-council claude 帮我设计这个后端仓库的新成员 onboarding 文档结构。重点考虑新人第一周真正需要知道什么。
```

## 安全边界

- 用户显式调用 `agent-council` 的 `council`、`claude` 或 `opencode` 模式时，即视为允许把任务包、提供的上下文和相关可读材料发送给对应外部 CLI Agent。
...
- `council`、`claude`、`opencode` 是同一个 skill 里的模式词，不是三个独立安装的 skill。
